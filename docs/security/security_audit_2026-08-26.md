...
| S-05 | Missing security headers | ✅ Fixed — HSTS, CSP, X-Frame-Options, Permissions-Policy, COOP, `no-store` on `/api`. CSP enforced on 2026-09-15 after the Report-Only period showed no violations (S-05b); `connect-src` also allows Google Fonts because html-to-image fetches them for dashboard captures | Caddy restart |
