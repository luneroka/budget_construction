...
|----|---------|--------|----------|
| S-01 | Public self-registration | ✅ Fixed — route removed; `POST /admin/users` + invite email; `app.scripts.create_admin` CLI; admin UI at *Paramètres → Utilisateurs* (invite, deactivate, delete, restore) | Live (admin UI deployed 2026-08-26 evening) |
...
| S-05 | Missing security headers | ✅ Fixed — HSTS, CSP, X-Frame-Options, Permissions-Policy, COOP, `no-store` on `/api`. CSP enforced on 2026-09-15 after the Report-Only period showed no violations (S-05b); `connect-src` also allows Google Fonts because html-to-image fetches them for dashboard captures. Same day, after the ZAP baseline: `Cross-Origin-Resource-Policy: same-origin` on every response and an explicit SPA cache policy (`no-cache` on `index.html`, one-year `immutable` on hashed `/assets/`) | Caddy restart |
