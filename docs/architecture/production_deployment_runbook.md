...
Provider, plan, IP addresses, OS and software versions, and the two provisioning
notes that used to be recorded here were moved to the gitignored
`docs/untracked/vps_information.md` on 2026-09-14, before the repository was made
public again. The public-facing facts: an Ubuntu 24.04 VPS in Germany running
Docker Compose, serving `batibudget.com`.
...
1. Cloned the repository to `~/budget_construction` on the VPS as `deploy`
   ...
   the repository was made private and the VPS switched to pulling over SSH
   with a read-only deploy key — see "GitHub access from the VPS" below.
   The repository was made public again on 2026-09-14; the VPS keeps the
   same SSH deploy-key setup, which works regardless of visibility.
...
**Update 2026-09-14:** the repository is public again (the infrastructure
details that motivated S-12 now live in the gitignored `docs/untracked/`).
Nothing below changes: deploy keys work on public repositories too, so the
VPS keeps pulling over SSH exactly as configured here.
...
| 2026-09-19 | v1.6.0  | UI consistency pass and budget accordion deployed (commits `4ca3e32`…`654f9ab`, frontend only; details in `docs/planning/ui_consistency_review_2026-09-19.md`): every field 40px and buttons sharing a row one height, button text sized by button size, the global CSS rules moved into `@layer base` (border colours now render), `--muted` 92%, the `!` workarounds removed; the budget page now opens one product and one sub-product at a time, outlines the open product and recentres on it. Pre-deploy backup `db-20260919T182340Z.sql.gz.enc`; only the frontend container was recreated (backend image unchanged, no migration); health endpoints green; the public login page checked in a browser (new field and button sizes, no CSP errors). Earlier the same day the PostgreSQL 18 migration had deployed `f28850b` (see "One-time: PostgreSQL 15 → 18"). |
| 2026-09-19 | v1.7.0  | Transaction flow and icon tooltips deployed (commits `0e08061`…`701aadb`, frontend only, with `a23aa82`: the help drawer's zone capture and its crop dependency removed): « + Devis » and « + Facture » in each transactions table's section rows, and an invoice icon on quote rows that prefills the invoice from the quote; a narrower transaction modal led by the document and the TTC amount (French-formatted, HT/TVA on demand), one-click statuses, and opening a transaction edits it in place; no more « Autoconstruction » label; the open product's name in bold; every icon-only control shows its label as an instant tooltip (`IconTooltips`, rule in `docs/planning/ui_consistency_review_2026-09-19.md`). Pre-deploy backup `db-20260919T204558Z.sql.gz.enc`; only the frontend container was recreated (no backend change, no migration); health endpoints green; the served bundle checked for the new texts; the public login page checked in a browser (the tooltip shows under the enforced CSP, only the expected 401 from the logged-out session refresh). |
| 2026-09-19 | v1.8.0  | Budget transactions side by side and unsaved-changes guard deployed (commits `cdb369c` and `cdb2e66`, frontend only): each product's quotes (« Devis ») and invoices (« Factures ») sit side by side once the product area is 78rem wide (stacked in one frame below, as before), in compact rows (supplier, then type and date) whose click opens the transaction; the eye icon is gone, the bin stays; both frames share their height and the add buttons fill the actions column. Every modal with a form (transactions, supplier, new project, sub-products, contact form) now asks « Abandonner les modifications ? » before its ×, Annuler or Fermer throws unsaved changes away. Pre-deploy backup `db-20260919T212556Z.sql.gz.enc`; only the frontend container was recreated (no backend change, no migration); health endpoints green; the served bundle checked for the new texts; on the public login page the contact modal opens and closes without a prompt when nothing is typed. |
| 2026-09-20 | v1.9.0  | Supplier pages deployed (commits `60197e5`…`a96e74d`): each supplier now has its own page at `/suppliers/:id` with its contacts (a « Voir plus » beyond two), quick access to write or copy the chosen contact's email and to open the RIB, a « Bilan » card (budget retenu, reste à payer, and a bar of what is invoiced against what was retained) and its transactions in the selected project; the supplier modal keeps create and edit only, with « Supprimer » inside it. A supplier's name opens that page from the suppliers, transactions and documents tables and from the dashboard's supplier chart; in the budget table it stays plain text, the row itself opening the transaction. Transactions tables show « Catégorie » and « Produit » in separate columns, the sub-product named under the product. The only backend change is `supplier_id` on each row of `GET /documents/` (repository, router, schema), so the backend image was rebuilt this time; no migration. Pre-deploy backup `db-20260920T073503Z.sql.gz.enc`; health endpoints green; the served bundle checked for the new texts; the public login page checked in a browser. |
...
| --- | --- | --- | --- |
| **High** | Dev/prod separation | **Partially resolved 2026-09-16: Resend now uses a dedicated dev key** (separate sending key on the same account, swapped into the local `.env`); the R2 half below is still open. The local root `.env` held **live, non-sandboxed** R2 and Resend credentials (real access keys, real `RESEND_API_KEY`), not dedicated dev/test ones. `backend/app/services/storage.py` has no environment-prefixing — a bug or accidental script run in local dev could write to, list, or delete objects in whatever bucket that key can reach, and/or send real email through the same Resend account. GitHub Actions already does this correctly (fake `test-bucket`/`test`/`test` credentials in `backend-tests.yml`); local dev does not. | Provision a dedicated dev-tier R2 bucket + a token scoped only to it, and use a Resend sandbox/test key (or a clearly separate low-volume sending identity) for local `.env` **(done 2026-09-16)**. Remaining Resend gap, **accepted 2026-09-16**: dev and prod still send from the same verified subdomain (`contact.luneroka.com`). The root domain is already insulated, and at this project's mail volume a stray bounce is noise rather than damage, so a separate `staging.` subdomain was judged not worth the DNS overhead. Mitigation is procedural and documented in `.env.example`: test mail goes only to a real inbox or to Resend's `delivered@`/`bounced@`/`complained@resend.dev` addresses, never to a made-up address. Revisit if mail-testing volume grows or if another project on the account starts sending in earnest. Separately, confirm on the VPS that `.env.production`'s `R2_BUCKET_NAME` differs from the local `.env`'s `budget-construction-documents` — don't paste the value into chat, just diff/grep on the server per the Chunk 4 lesson learned. |
