"""move selected budget to budget lines

Revision ID: b7e4c8d9a123
Revises: 0f4b2d9c8a1e
...
revision: str = 'b7e4c8d9a123'
down_revision: Union[str, Sequence[str], None] = '0f4b2d9c8a1e'
...
def _selected_budget_line_view_sql(filename: str) -> str:
    return (
...
def _drop_selected_budget_trigger() -> None:
    op.execute(
...
def upgrade() -> None:
    ...
    op.execute("""
        ...
        SET selected_budget_transaction_id = selected_transaction.id
        FROM (
            SELECT DISTINCT ON (t.budget_line_id)
                t.id,
                t.budget_line_id
            FROM transactions AS t
            WHERE t.is_selected_budget IS TRUE
              AND t.deleted_at IS NULL
              AND t.transaction_type IN ('quote', 'diy_estimate')
            ORDER BY t.budget_line_id, t.id
        ) AS selected_transaction
        WHERE bl.id = selected_transaction.budget_line_id
          AND bl.deleted_at IS NULL
        """)

    op.drop_index(
        'uq_transactions_budget_line_selected_budget', table_name='transactions'
    )
    op.drop_constraint(
    ...
    for filename in ANALYTICS_SQL_FILES:
        op.execute(_selected_budget_line_view_sql(filename))
...
def downgrade() -> None:
    ...
    op.execute("""
        ...
        WHERE bl.selected_budget_transaction_id = t.id
          AND bl.deleted_at IS NULL
          AND t.deleted_at IS NULL
        """)
    ...
    op.create_index(
        ...
        ['budget_line_id'],
        unique=True,
        postgresql_where=sa.text('is_selected_budget AND deleted_at IS NULL'),
    )

    _drop_selected_budget_trigger()
    op.drop_index(
        'uq_budget_lines_selected_budget_transaction',
